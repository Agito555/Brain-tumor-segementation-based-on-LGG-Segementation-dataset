...
from readin import Brain_dataset,make_dir,select_data,find_m_and_v
    ...
    for phase in ['train','test']:
        ...
        for image, mask in now_loader:
            ...
            with torch.set_grad_enabled(phase=='train'):
                if phase=='train':
                    prediction,prediction2 = unet(image)
                    loss = 0.5*Loss_dsc(prediction, mask)+0.5*Loss_dsc(prediction2,mask)#try one loss firstly,
                                            #in the seconde stage,i will attempt to
                                            #add one more loss:CE
                else:
                    prediction=unet(image)
                    loss=Loss_dsc(prediction, mask)
            if phase=='train':
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
...
#         mask = mask.to(device, dtype=torch.float)
#         prediction = unet(image)
#         running_dsc+=dsc(prediction,mask)*prediction.shape[0]
#         for i in range(prediction.shape[0]):
#             pred=transforms.ToPILImage()(prediction[i])
#             msk=transforms.ToPILImage()(mask[i])
#             img=transforms.ToPILImage()(image[i])
#             plt.subplot(1, 3, 1)
#             plt.imshow(pred)
#             plt.subplot(1, 3, 2)
#             plt.imshow(msk)
...
#             plt.imshow(img)
#             plt.show()
#             print('{} test dsc: {}'.format(i, dsc(prediction, mask)))
#     epoch_dsc=running_dsc/number
# print('average {} dsc:{}'.format('test', epoch_dsc))
